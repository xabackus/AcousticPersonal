...
paused = False
N = 60  # Number of particles (changed to 60 for equal splitting into 3 clusters)
...
def run_simulation_with_actions(actions):
    global pos, vel, kx_field, ky_field, key_press_data_indices, average_neighbors_over_time, neighbor_count_over_time, weighted_avg_density_over_time, weighted_avg_aspect_ratio_over_time, time_step, cluster_id
...
def optimize_actions():
    ...
    for iteration in range(num_iterations):
        ...
        current_loss = run_simulation_with_actions(actions)
        ...
        for i in range(num_actions):
            for j in range(2):  # For kx and ky
                ...
                loss_plus = run_simulation_with_actions(actions)

                # Negative perturbation
                actions[i][j] = original_value - delta
                loss_minus = run_simulation_with_actions(actions)
    ...
    # After optimization, run the simulation one more time with optimized actions
    final_loss = run_simulation_with_actions(actions)
